...
Dữ liệu từ Steam Games Dataset (Kaggle) · Pipeline: MongoDB → PySpark → Parquet → dbt → DuckDB
""")
st.divider()
...
def load_kpis():
    return con.execute("""
        SELECT
            ...
            (SELECT COUNT(DISTINCT genre_id) FROM dim_genre)      AS total_genres,
            SUM(f.positive + f.negative)                          AS total_reviews,
            ROUND(AVG(CASE WHEN d.price > 0 THEN d.price END), 2) AS avg_price
...
c4.metric("Giá trung bình",    f"${float(kpi['avg_price']):.2f}")
st.divider()
...
selected_genres = st.multiselect(
    "Chọn thể loại muốn hiển thị:",
    options=sorted(df_bq1["genre_name"].unique().tolist()),
    default=top8_default,
    key="bq1_filter"
)

if selected_genres:
    df_bq1_plot = df_bq1[df_bq1["genre_name"].isin(selected_genres)]
    fig1 = px.line(
        df_bq1_plot,
        x="release_year",
        y="game_count",
...
st.markdown("##### Top 10 genre có tổng số game nhiều nhất (2000–2024)")
df_bq1_summary = (
    df_bq1.groupby("genre_name")["game_count"]
    .sum().reset_index()
    .rename(columns={"genre_name": "Thể loại", "game_count": "Tổng số game"})
    .sort_values("Tổng số game", ascending=False)
    .head(10).reset_index(drop=True)
)
df_bq1_summary.index += 1
st.dataframe(df_bq1_summary, use_container_width=True)

with st.expander("Insight BQ1"):
    st.markdown("""
    - **Indie** là thể loại có số lượng game lớn nhất, tăng mạnh từ 2012 đến nay — nhưng cũng **cạnh tranh cao nhất**.
    - **Action**, **Adventure**, **Casual** tăng đều và ổn định.
    - Hầu hết genre đạt đỉnh quanh **2022–2023** rồi giảm nhẹ.
    - **Khuyến nghị:** Ưu tiên các genre có supply tăng nhưng chưa bão hòa như Action, RPG, Simulation.
    """)
...
def load_bq2_scatter():
    """Group theo từng mức giá — dùng cho bubble chart"""
    return con.execute(f"""
        SELECT
            ...
            ROUND(AVG({OWNERS_MIDPOINT}), 0) AS avg_owners,
            COUNT(*) AS game_count
        FROM fact_game_stats f
        JOIN dim_game d  USING (game_id)
...
@st.cache_data
def load_bq2_bucket():
    """Group theo khoảng giá — dùng cho bảng tóm tắt"""
    return con.execute(f"""
        SELECT
            CASE
                WHEN d.price = 0       THEN '① Free (= $0)'
                WHEN d.price <= 5      THEN '② $0.01 – $5'
            ...
            ROUND(
                AVG(CAST(f.positive AS DOUBLE) / (f.positive + f.negative)) * 100,
            1)                                               AS avg_positive_rate_pct,
            ROUND(AVG({OWNERS_MIDPOINT}), 0)                 AS avg_owners_estimate
        FROM fact_game_stats f
        JOIN dim_game d  USING (game_id)
        JOIN stg_game s  USING (game_id)
...
df_scatter = load_bq2_scatter()
df_bucket  = load_bq2_bucket()
...
with col_chart:
    fig2 = px.scatter(
        ...
        color_discrete_sequence=["#F789AF"]
    )
...
with col_table:
    ...
    df_bucket_display = df_bucket.rename(columns={
        "price_bucket":          "Khoảng giá",
        "game_count":            "Số game",
        "avg_positive_rate_pct": "Review tích cực TB (%)",
        "avg_owners_estimate":   "Người sở hữu TB ước tính"
    })
    st.dataframe(df_bucket_display, use_container_width=True, hide_index=True)

with st.expander("Insight BQ2"):
    st.markdown("""
    - Khoảng **$5–$20** thường có tỷ lệ review tích cực cao và lượng người sở hữu lớn — **sweet spot** cho indie.
    - Game **free-to-play** có lượng người sở hữu rất cao nhưng review tích cực thấp hơn paid games.
    - Game **>$30** có ít người sở hữu hơn, phù hợp AAA hơn indie.
    - **Khuyến nghị:** Định giá trong khoảng **$9.99–$19.99** để cân bằng accessibility và perceived value.
    """)
...
st.markdown("""
**Phương pháp:** Tổng số người sở hữu ước tính (dùng midpoint của mỗi range) theo từng genre.  
Màu sắc = số lượng game → phân biệt **demand** (người chơi) vs **supply** (số game).
""")
...
def load_bq3():
    return con.execute(f"""
        SELECT
            g.genre_name,
            SUM({OWNERS_MIDPOINT})      AS total_estimated_owners,
            COUNT(DISTINCT s.game_id)   AS game_count,
            ROUND(AVG({OWNERS_MIDPOINT}), 0) AS avg_owners_per_game
        FROM stg_game s
...
fig3.update_layout(xaxis_tickangle=-40, coloraxis_colorbar_title="Số game")
...
with st.expander("Insight BQ3"):
    st.markdown("""
    - **Indie** dẫn đầu nhờ volume lớn, nhưng **cạnh tranh cao nhất**.
    - **Action**, **Adventure** có tổng người chơi lớn với số game ít hơn → tỷ lệ người chơi/game cao.
    - Xem cột **"Người sở hữu TB / game"** để tìm genre nào hiệu quả nhất.
    - **Khuyến nghị:** Genre có `avg_owners_per_game` cao nhưng `game_count` thấp = blue ocean opportunity.
    """)
...
st.markdown("""
Kết hợp BQ1 và BQ3: **trục X** = số game được publish (supply) · **trục Y** = tổng người sở hữu ước tính (demand).  
**Góc trên bên trái** = demand cao, supply thấp → **cơ hội tốt nhất** cho indie developer.
""")
...
def load_matrix():
    return con.execute(f"""
        SELECT
            g.genre_name,
            COUNT(DISTINCT f.game_id)   AS total_games,
            SUM({OWNERS_MIDPOINT})      AS total_owners
        FROM fact_game_stats f
        JOIN stg_game s          USING (game_id)
        JOIN bridge_game_genre b USING (game_id)
        JOIN dim_genre g         USING (genre_id)
        GROUP BY g.genre_name
        HAVING COUNT(DISTINCT f.game_id) >= 50
...
fig4 = px.scatter(
    ...
    color_discrete_sequence=["#EF7ED3"]
)
fig4.update_traces(textposition="top center", textfont_size=10)
fig4.update_layout(showlegend=False)
st.plotly_chart(fig4, use_container_width=True)

st.caption("Genre ở góc trên bên trái (demand cao, supply thấp) = cơ hội tốt nhất. Genre ở góc dưới bên phải = thị trường bão hòa.")

st.divider()
st.caption("Steam Games Dataset (Kaggle) | Pipeline: MongoDB → PySpark → Parquet → dbt → DuckDB")
